...
export default function Sidebar({ currentView, setCurrentView }: SidebarProps) {
    ...
    return (
        <>
            <aside className="w-full md:w-[320px] lg:w-[350px] flex flex-col p-6 animate-[slideInLeft_0.5s_ease] relative glass border-t border-l border-[var(--border)] h-[100dvh] md:h-full md:rounded-2xl shrink-0 transition-colors duration-500 overflow-hidden">
                <div className="flex items-center gap-3 text-2xl font-bold mb-8 tracking-wide text-[var(--text-main)] transition-colors duration-500">
                    <Logo size={40} className="drop-shadow-[0_0_10px_var(--primary)] transition-all duration-500" />
                    OPAS
                </div>

                {/* Navigation Links */}
                <div className="flex flex-col gap-2 mb-8 font-medium">
                    <button onClick={() => setCurrentView('dashboard')} className={`p-3 rounded-lg flex items-center gap-3 transition-all duration-300 w-full text-left ${currentView === 'dashboard' ? 'bg-[var(--accent)] border border-[var(--hover)] text-[var(--text-main)] drop-shadow-[0_0_8px_var(--primary)]' : 'text-[var(--text-muted)] hover:text-[var(--text-main)] hover:bg-[var(--accent)] border border-transparent'}`}>
                        <i className={`fa-solid fa-satellite-dish w-5 text-center ${currentView === 'dashboard' ? 'text-[var(--primary)]' : ''}`}></i> Operations Board
                    </button>
                    <button onClick={() => setCurrentView('integrations')} className={`p-3 rounded-lg flex items-center gap-3 transition-all duration-300 w-full text-left ${currentView === 'integrations' ? 'bg-[var(--accent)] border border-[var(--hover)] text-[var(--text-main)] drop-shadow-[0_0_8px_var(--primary)]' : 'text-[var(--text-muted)] hover:text-[var(--text-main)] hover:bg-[var(--accent)] border border-transparent'}`}>
                        <i className={`fa-solid fa-key w-5 text-center ${currentView === 'integrations' ? 'text-[var(--primary)]' : ''}`}></i> API & Integrations
                    </button>
                    <button onClick={() => setCurrentView('knowledge')} className={`p-3 rounded-lg flex items-center gap-3 transition-all duration-300 w-full text-left ${currentView === 'knowledge' ? 'bg-[var(--accent)] border border-[var(--hover)] text-[var(--text-main)] drop-shadow-[0_0_8px_var(--primary)]' : 'text-[var(--text-muted)] hover:text-[var(--text-main)] hover:bg-[var(--accent)] border border-transparent'}`}>
                ...
                <button
                    ...
                    className="w-full p-4 bg-transparent border border-dashed border-[var(--hover)] text-[var(--primary)] font-semibold rounded-xl hover:bg-[var(--accent)] hover:border-solid hover:shadow-[0_0_20px_var(--accent)] transition-all mb-6 flex items-center justify-center gap-2"
                >
                    <i className="fa-solid fa-plus drop-shadow-[0_0_8px_var(--primary)]"></i> Delegate Task
                </button>
                ...
                <div className="flex-grow overflow-y-auto hide-scrollbar flex flex-col gap-3 pr-2 mb-4">
                    {tasks.length === 0 ? (
                        ...
                        tasks.map(t => (
                            <div
                                key={t.id}
                                className={`p-4 rounded-xl bg-[var(--bg)]/40 border-l-4 cursor-pointer hover:bg-[var(--bg)]/60 transition-all shadow-md
                  ${t.status === 'queued' ? 'border-[var(--text-muted)]' :
                                        t.status === 'running' ? 'border-[var(--primary)] bg-[var(--accent)]' :
                                            t.status === 'waiting_for_user' ? 'border-[#f39c12] bg-[#f39c12]/5' :
                                                'border-[#10a37f] bg-[#10a37f]/5'}`
                                }
                            >
                                <div className="text-[10px] font-bold tracking-wider uppercase px-2 py-1 rounded bg-[var(--panel)] border border-[var(--border)] mb-3 inline-block text-[var(--text-main)]">
            ...
            {isModalOpen && (
                <div className="fixed inset-0 bg-black/60 backdrop-blur-md flex items-center justify-center z-50 p-4">
                    <div className="glass w-full max-w-[550px] p-8 flex flex-col gap-6 animate-[popIn_0.3s_ease] border border-[var(--primary)]/30 shadow-[0_0_50px_var(--accent)] rounded-2xl max-h-[90vh] overflow-y-auto hide-scrollbar bg-[var(--bg)]">
                        <div>
                            <h2 className="text-2xl font-bold m-0 flex items-center gap-2 text-[var(--text-main)]"><i className="fa-solid fa-bolt text-[var(--primary)] drop-shadow-[0_0_8px_var(--primary)]"></i> Agentic Delegation</h2>
                        ...
                        <form className="flex flex-col gap-5" onSubmit={async (e) => {
                            ...
                            <div className="flex flex-col gap-1.5">
                                ...
                                <input name="title" required type="text" className="p-3 bg-black/20 border border-[var(--border)] rounded-lg text-[var(--text-main)] outline-none focus:border-[var(--primary)] transition-colors" placeholder="e.g. University Applications" />
                            </div>
                            <div className="flex flex-col gap-1.5">
                                <label className="text-[11px] font-bold text-[var(--text-muted)] uppercase tracking-widest">Detailed Objective & Payload</label>
                                <textarea name="description" required rows={4} className="p-3 bg-black/20 border border-[var(--border)] rounded-lg text-[var(--text-main)] outline-none focus:border-[var(--primary)] transition-colors resize-none" placeholder="Provide context, URLs, and overall goals..."></textarea>
                            </div>
                            <div className="flex gap-4">
                                <div className="flex flex-col gap-1.5 flex-1">
                                    <label className="text-[11px] font-bold text-[var(--text-muted)] uppercase tracking-widest">Deadline</label>
                                    <input name="deadline" type="date" className="p-3 bg-black/20 border border-[var(--border)] rounded-lg text-[var(--text-main)] outline-none focus:border-[var(--primary)] transition-colors font-mono text-sm" />
                                </div>
                                <div className="flex flex-col gap-1.5 flex-1">
                                    <label className="text-[11px] font-bold text-[var(--text-muted)] uppercase tracking-widest">Token Budget Limit</label>
                                    <input name="budget" required type="number" defaultValue={50000} className="p-3 bg-black/20 border border-[var(--border)] rounded-lg text-[var(--text-main)] outline-none focus:border-[var(--primary)] transition-colors font-mono text-sm" />
                                </div>
                            </div>

                            <div className="flex justify-end gap-3 mt-2">
                                <button type="button" onClick={() => setIsModalOpen(false)} className="px-6 py-2.5 rounded-lg bg-transparent border border-[var(--border)] text-[var(--text-main)] font-semibold hover:bg-[var(--accent)] transition-colors">Cancel</button>
                                <button type="submit" className="px-6 py-2.5 rounded-lg bg-[var(--primary)] text-black font-bold hover:opacity-90 transition-opacity shadow-[0_0_20px_var(--accent)]">Launch Operation</button>
                            </div>
                        </form>
                    </div>
                </div>
